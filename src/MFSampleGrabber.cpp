...
namespace SIPSorceryMedia
{
  ...
  {
    int mediaType = IsEqualGUID(guidMajorMediaType, MFMediaType_Video) ? VIDEO_TYPE_ID : AUDIO_TYPE_ID; // TODO double check...    
  ...
  HRESULT MFSampleGrabber::Run(System::String^ mediaPath, bool loop)
  {
    ...
    SampleGrabberCB* pAudioSampleGrabberSinkCallback = NULL, * pVideoSampleGrabberSinkCallback = NULL;
    ...
    pAudioSampleGrabberSinkCallback->SetHandlers(cbClockStart, cbProcessSample);
    pVideoSampleGrabberSinkCallback->SetHandlers(cbClockStart, cbProcessSample);
    ...
    GCHandle gchVidResChanged = GCHandle::Alloc(vidResChangedDelegate); // Stop delegate from being garbage collected. TODO: gch.Free(); when finished.
...
HRESULT SampleGrabberCB::CreateInstance(SampleGrabberCB** ppCB, REFGUID mediaType)
{
...
STDMETHODIMP SampleGrabberCB::OnProcessSample(REFGUID guidMajorMediaType, DWORD dwSampleFlags,
  ...
  DWORD dwSampleSize)
{
  // The guidMajorMediaType is not being set and always equals GUID_NULL.
  ...
  _onProcessSampleFunc(_mediaType, dwSampleFlags, llSampleTime, llSampleDuration, pSampleBuffer, dwSampleSize);
  return S_OK;
}
...
HRESULT CreateTopology(IMFMediaSource* pSource, IMFActivate* pVideoSinkActivate, IMFActivate* pAudioSinkActivate, IMFTopology** ppTopo)
{
  ...
  for (DWORD i = 0; i < cStreams; i++)
  {
    ...
    else if (majorType == MFMediaType_Audio && fSelected)
    {
      ...
      // TODO: Should be possible to add the MULAW codec here using AddTransformNode, 
      // see https://docs.microsoft.com/en-us/windows/win32/medfound/adding-a-decoder-to-a-topology.
